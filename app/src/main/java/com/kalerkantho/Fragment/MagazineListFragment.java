    ...
    private void intiU() {
        ...
        try {
            ...
            if (!(TextUtils.isEmpty(PersistData.getStringData(con, AppConstant.CATEGORY_RESPONSE)))){
                ...
                allCategory = g.fromJson(PersistData.getStringData(con, AppConstant.CATEGORY_RESPONSE),MagazineCategory.class);
                if (onlineList!=null)
                    onlineList.clear();
                ...
                for (int i=0;i<allCategory.getCategory_list().size();i++){
                    if (allCategory.getCategory_list().get(i).getM_type().equalsIgnoreCase("feature")){
